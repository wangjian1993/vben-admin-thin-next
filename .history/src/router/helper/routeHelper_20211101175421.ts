...
function asyncImportRoute(routes: AppRouteRecordRaw[] | undefined) {
  ...
  routes.forEach((item) => {
    ...
    const { children } = item;
    if (component) {
      console.log('1111111====', item.component);
      ...
      console.log('222222====', item.component);
...
export function transformObjToRoute<T = AppRouteModule>(routeList: AppRouteModule[]): T[] {
  ...
  routeList.forEach((route) => {
    ...
    // } else {
    //   warn('请正确配置路由：' + route?.name + '的component属性');BlankView
    // }
    if (component.toUpperCase() === 'BlankView' || component == '') {
      console.log('222');
