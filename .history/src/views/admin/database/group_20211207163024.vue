<!--
 * @Author: max
 * @Date: 2021-11-11 13:57:09
 * @LastEditTime: 2021-12-07 16:29:55
 * @LastEditors: max
 * @Description: 
 * @FilePath: /vben-admin-thin-next-main/src/views/admin/database/group.vue
-->
<template>
  <div>
    ...
    <AppTypeForm @register="registerModal" @success="handleSuccess" />
...
<script lang="ts">
  ...
  import { columns, searchFormSchema } from './data/group';
  import { getParamGroupList, paramGroupAction } from '/@/api/system/system';
  import { Tag } from 'ant-design-vue';
  import AppTypeForm from './component/AppTypeForm.vue';
  ...
  export default defineComponent({
    components: { BasicTable, Tag, TableAction, AppTypeForm, Drawer },
    setup() {
      //弹出框
      ...
      async function handleSuccess(item) {
        if (!item.isUpdate) {
          await paramGroupAction(item.values, 'add');
          createMessage.success('新增成功！');
        } else {
          await paramGroupAction(item.values, 'update');
      ...
      async function handleDelete(record: Recordable) {
        let parmas: string[] = [];
        parmas.push(record.ParamGroupId);
        await paramGroupAction(parmas, 'delete');
      ...
      async function handleAllDelete() {
        createConfirm({
          ...
          content: '确认要删除选中内容？',
          onOk: async () => {
            await paramGroupAction(checkedKeys.value, 'delete');
            createMessage.success('删除成功！');
            await reload();
          },
        });
